...
export function RequirementsInput({ onGenerate, error }: Props) {
  ...
  const [hasEnvKey, setHasEnvKey] = useState<boolean | null>(null) // null = loading

  // Check if server has ANTHROPIC_API_KEY configured
  useEffect(() => {
    fetch('/api/has-api-key')
      .then(r => r.json())
      .then((d: { hasApiKey: boolean }) => setHasEnvKey(d.hasApiKey))
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!hasEnvKey && !apiKey.trim()) return
    onGenerate(requirements, apiKey)
  }

  const canSubmit = requirements.trim().length > 0 && (hasEnvKey === true || apiKey.trim().length > 0)
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 flex items-center justify-center p-6">
      <div className="w-full max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* API Key - hidden when env var is set */}
          {hasEnvKey === true ? (
            <div className="bg-green-500/10 border border-green-500/30 rounded-2xl px-5 py-3 flex items-center gap-3">
              ...
              <div>
                <span className="text-green-300 text-sm font-medium">APIキーが環境変数から読み込まれています</span>
                <span className="text-green-600 text-xs block">ANTHROPIC_API_KEY (.env)</span>
              </div>
            </div>
          ) : hasEnvKey === false ? (
            <div className="bg-slate-800/60 backdrop-blur border border-slate-700 rounded-2xl p-5">
              ...
              <p className="text-xs text-slate-500 mt-2">
                APIキーはサーバーに保存されません。リクエスト時のみ使用されます。
                <br />
                ヒント: プロジェクトの <code className="text-slate-400">.env</code> に <code className="text-slate-400">ANTHROPIC_API_KEY=sk-ant-...</code> を設定すると入力不要になります。
              </p>
            </div>
          ) : (
            // Loading state
            <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-4 animate-pulse">
